...
- Culoarea apare pentru că *ghidează atenția*, sau pentru că "arată bine"?
...
Testul practic pentru orice element: *"E acesta ceva pe care vreau ca utilizatorul să-l observe activ sau să-l apese acum?"*
...
**Container global:** `max-w-[1320px]`, `px-4` (+ `lg:px-8` opțional pe ecrane foarte late). Toate secțiunile majore folosesc *exact* aceeași lățime — marginile trebuie să cadă aliniate vertical pe toată pagina. Excepții intenționate și explicite:
...
| **M — module de capabilitate** | Services | Ferestre mai mici, una per serviciu/capabilitate, fiecare demonstrând *un singur lucru* (nu un dashboard complet — un fragment de UI relevant pentru acel serviciu). Interacțiune la hover (nu doar float pasiv) — utilizatorul simte că "atinge" produsul. |
...
Regulă comună tuturor celor trei scări (ce le face o *familie*, nu piese disparate):
...
| **Moment-semnătură** (per secțiune, vezi §1.2) | poate depăși aceste praguri *cu justificare explicită* | — | maximum o dată per secțiune |
...
**Reguli:** stroke-width implicit (2px). Dimensiuni: `h-3.5 w-3.5` (chip-uri mici), `h-4 w-4` (butoane/liste), `h-5 w-5` (nav, elemente izolate). Monocrome implicit — `text-primary` doar când iconița *este* accentul curent (conform §2.2), altfel `text-muted-foreground`.
